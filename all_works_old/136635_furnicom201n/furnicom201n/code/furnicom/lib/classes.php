...
if ( !class_exists('Furnicom_Config')):
class Furnicom_Config {
	...
	public static function setVariables($vars){
		if ( is_null(self::$instance) ){
			self::$instance = new Furnicom_Config();
...
if ( !class_exists('Furnicom_Widget') ):

abstract class Furnicom_Widget extends WP_Widget{
	...
	protected function init(){
		...
		$this->base_tpl_path = apply_filters('furnicom_widget_template_base', $this->base_path.'/widgets/'.$this->id_base.'/tmpl');
		$this->override_tpl_path = apply_filters('furnicom_widget_override_base', get_template_directory().'/widgets/'.$this->id_base);
	}
	...
	public function furnicom_trim_words( $text, $num_words = 30, $more = null ) {
	...
	public function widget_template_select( $field_name, $opts = array(), $field_value = null ){
		...
		if ( array_key_exists('ov', $templates) && count($templates['ov']) ){
			$select_html .= '<optgroup label="'. esc_html__('Override by Theme', 'furnicom') .'">';
		...
		if ( array_key_exists('df', $templates) && count($templates['df']) ){
			$select_html .= '<optgroup label="'. esc_html__('Default Template', 'furnicom') .'">';
	...
	public function form( $instance ){
		...
		?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('title') ); ?>"><?php esc_html_e('Title', 'furnicom')?></label>
			<br />
			<input class="widefat"
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('widget_template') ); ?>"><?php esc_html_e("Template", 'furnicom')?></label>
			<br/>
